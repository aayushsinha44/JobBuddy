from user.models import Recruiter
...
class Recruiter:
    ...
    def create_user(user_structure):
        ...
        if Recruiter.objects.filter(email_id=user_structure["email"],
                                    phone_number=user_structure["phone_number"]).count() > 0:
            return USER_EXISTS

        # Add user
        Recruiter.objects.create(first_name=user_structure["first_name"],
